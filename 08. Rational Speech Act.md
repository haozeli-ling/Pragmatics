...
- [Frank & Goodman (2012)](https://www.science.org/doi/10.1126/science.1218633) Predicting Pragmatic Reasoning in Language Games. *Science*. 
- [Scontras et al. (2021)](https://arxiv.org/abs/2105.09867) A practical introduction to the Rational Speech Act modeling framework
...
| Relevance | Question under Discussion  |
...
Utterance space: $U =$ { NONE, SOME, ALL }
- NONE : 'I ate none of the cookies.'
- SOME : 'I ate some of the cookies'
- ALL : 'I ate all of the cookies'

Semantics: We model the literal meaning of an utterance as the set of situations verifying this utterance. 
- the meaning of NONE is { $s_0$ }
- the meaning of SOME is { $s_1, s_2, s_3, s_4$  }
- the meaning of ALL is { $s_4$ }
...
> [!IMPORTANT]
>
> $P_{L_0} (s | u) = \displaystyle \frac{ ⟦u⟧(s) \cdot P(s) }{ \Sigma_{s' \in S} ⟦u⟧(s') \cdot P(s')  }$
> 
...
Reasoning about the literal listener informs the pragmatic speaker rule: 
...
> - Let's jump to [Session 9](https://github.com/haozeli-ling/Pragmatics/blob/main/09.%20Informativeness%20in%20RSA.md) for the details of utility functions. 

The exponential function enables soft-maximization, allowing the RSA speaker to consider multiple utterance options rather than being restricted to the single highest-utility choice. For simplicity, let's assume that producing any utterance incurs no significant cost ($\text{Cost}'(u) = 0$) and that $\alpha = 1$. 
