...
**Example**: Suppose you are live in Singapore, ... 
- $u_1 =$ 'it will rain today': $P(u_1) = 0.8$. Hearing this adds little new information since you already expect rain. 
...
> [!NOTE]
>
> Intuitively, $\color{red}{\text{a low-probability utterance is more informative precisely because it is unexpected}}$ . If a speaker chooses an expression that they rarely use, that choice calls for an explanation: the listener reasons that the speaker must have had a good reason for deviating from the more typical, high-probability alternatives.
> 
> - Common utterances carry little new information. They just say what we already anticipated.
> - Uncommon utterances significantly reshape the listener’s beliefs.
>
> Hearing something surprising prompts the listener to update more strongly, inferring that the underlying situation must be special enough to justify such an unusual choice. In everyday terms, when someone goes out of their way to say something in a marked or unlikely manner, we naturally conclude that they are trying to convey something more specific or stronger than what a routine utterance would have communicated.
