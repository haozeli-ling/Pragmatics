...
### What kind of meaning it this

This meaning is not part of literal meaning or entailment, but belong to social meaning: 

> [!IMPORTANT]
> **Social Meaning**: the aspect of meaning through which linguistic forms signal information about the speaker’s identity, stance, and social positioning.

Labov established a correlation between linguistic variants and social groups: '-in' occurs more frequently among working-class speakers. However, later work argues that this is only the surface pattern. The deeper generalization is: 

> [!IMPORTANT]
> Linguistic variants index social meanings, which listeners use to construct inferences about speakers.

Social meaning arises through indexicality: A linguistic form points to (indexes) social meanings via culturally established associations. 

- 'ing' -- competent, aloof
- '-in' -- incompetent, friendly

From a pragmatic perspective, what listeners do is inferring the persona of the speaker based on the used forms and context. 

- Hearing 'walkin' in a causal setting -> the speaker is relaxed or friendly
- Gearing 'walkin' in a formal speech -> the speaker is performing authenticity. 

These social meanings are not fixed and depends on context. 

People’s production of language is sensitive to such social meanings and the context. Labov (2012) analyzes President Obama’s use of '-ing' or '-in' across three recordings, each captured in a context with a different level of formality.
...
$ST(u | i)$: how likely a stereotypical speaker with persona $i$ would produce $u$
...
$P_{S_1} ('in' | \text{Gool Guy}) = \displaystyle \frac{\exp(6 \cdot \text{Utility}'('in', \text{Gool Guy}))}{\Sigma_{u' \in U}\ \exp(6 \cdot \text{Utility}'(u', \text{Gool Guy}))} = 0.5772$

$P_{S_1} ('ing' | \text{Gool Guy}) = \displaystyle \frac{\exp(6 \cdot \text{Utility}'('ing', \text{Gool Guy}))}{\Sigma_{u' \in U}\ \exp(6 \cdot \text{Utility}'(u', \text{Gool Guy}))} = 0.4228$
