Great question! The use of the **logarithm (\(\log\))** in the utility function in frameworks like the Rational Speech Act (RSA) model is motivated by several mathematical, theoretical, and practical reasons. The utility function often looks like this:

\[
\text{Utility}(u, s) = \log P_{L_0}(s \mid u) - \text{Cost}(u)
\]

Here, the term \(\log P_{L_0}(s \mid u)\) captures the informativeness of the utterance \(u\). Let’s break down why \(\log\) is used instead of just \(P_{L_0}(s \mid u)\) (the literal listener’s probability).

---

### 1. **Additivity of Information (Logarithms and Information Theory)**
The logarithm is **intimately connected to information theory**, where it measures **information content** or **surprisal**. Specifically:

\[
\text{Surprisal} = -\log P_{L_0}(s \mid u)
\]
